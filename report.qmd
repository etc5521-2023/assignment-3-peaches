...
In preparation for the forthcoming analysis, an Initial Data Analysis (IDA) was conducted by removing a few unnecessary columns and identifying and systematically saving only the wild sightings.
...
Utilising an interactive Leaflet map of Woylie sightings allows for the identification of the nearest weather stations to the clusters of Woylie sightings. As demonstrated in @fig-cluster-map, the five closest weather stations are:
...
The highest number of Woylie sightings was recorded in Pilliga East State Forest, and consequently, the weather data was obtained from the nearest weather station, Narrabri Airport. The weather data can then be merged with Woylie observation data to create a comprehensive dataset for use in @sec-ref-1.
...
- Population Relocation: Facilitating the relocation of Woylies to conservation areas to establish additional populations, a strategy designed to mitigate the risk of extinction. This can be observed in Expectation 3, where the majority of Woylie clusters are in national parks.
...
- Disease Management Programs: Implementing proactive disease management programs, which include vaccination initiatives for Woylies and vigilant population monitoring to detect and respond to potential disease outbreaks, as evidenced by Expectation 1, which reveals a recent spike in Woylie observations.
...
```{r}
#| label: fig-narr
#| fig-cap: "Number of Holiday Trips in Narrabri Surrounds"
