...
Using the leaflet interactive maps we identified the closest weather
station to the clusters of woylie sightings.
...
# Print the number of sightings in each cluster
cluster_counts %>% 
...
```{r}
# Normalize the counts for radius scaling
max_count <- max(cluster_counts$num_sightings)
normalized_counts <- cluster_counts$num_sightings / max_count

# Scale the radius based on the normalized counts
scaled_radius <- normalized_counts * 20  # Adjust 20 as needed for appropriate scaling

# Create a Leaflet map
map_clusters <- leaflet(data = WILD_THING_woylie) %>%
  addTiles()  # Add a basemap (you can customize the basemap provider)

# Add circle markers for each sighting with scaled radius
map_clusters <- map_clusters %>%
  addCircleMarkers(
    lng = ~longitude,  # Longitude
    lat = ~latitude,    # Latitude
    popup = ~paste("Sighting Date: ", format(eventDate, "%Y-%m-%d")),  # Popup text
    radius = ~scaled_radius[cluster],  # Use scaled radius based on the cluster
    color = "blue",  # Marker color
    fillOpacity = 0.7  # Marker fill opacity
  )
...
woylie_stations <- woylie_stations %>% 
  distinct(recordID, .keep_all = TRUE)  # .keep_all = TRUE keeps all columns
