...
Ecotourism involves traveling to places in support of the conservation efforts of endangered wildlife. To assess the level of interest in ecotourism for the Woylie, an analysis that combines tourism data and previous findings will provide us with a holistic view. In this initial ecotourism analysis, the focus will be on one of the prominent regions where Woylies are found: Port Lincoln in the Port Lincoln National Park.

## Port Lincoln, Lincoln National Park

### Analysis 

The 'tourism' data will be filtered for the region of **Port Lincoln** as part of the analysis. The purpose of tourism will also be filtered to **holiday** since this suggests the intention to travel for sightseeing and exploration. Other purposes, such as visiting friends and relatives or business, may include visiting the endangered Woylie species during the trip, but the purposes are too broad to draw conclusive insights. Therefore, choosing **holiday** tourism data will result in the most accurate analysis.

As previous findings have shown, the number of observations of Woylies were most prominent in the year 2022. Most observations were made during the Spring and Summer months (starting in October). Therefore, a deeper analysis will focus on Quarter 4 of the year in relation to tourism numbers. Most importantly, the question that is explored is, is there a correlation between the number of observations and the number of tourists in the region?

```{r}
set.seed(383724)
kmeans_result <- kmeans(coords_matrix, centers = num_clusters)
WILD_THING_woylie$cluster <- kmeans_result$cluster
port_lincoln_observations <- WILD_THING_woylie %>%
  filter(cluster == 2) %>%
  mutate(Quarter = yearquarter(eventDate)) %>% 
  select(Quarter, everything())
```

```{r}
port_lincoln_tourism <- tourism %>%
  filter(Region == "Port Lincoln",
         Purpose == "Holiday") %>% 
...
```{r}
ggplot(port_lincoln_tourism, aes(x = Quarter, y = Trips, fill = as.factor(Q))) +
  geom_col() +
  facet_wrap(~ Year, scales = "free_x") +
  scale_x_discrete(labels = scales::date_format("%Y")) +
  labs(title = "Holiday Tourism in Port Lincoln by Year",
       x = "Quarter",
       y = "Number of trips") +
  theme_bw() +
...
```{r}
port_lincoln_full <-
  left_join(port_lincoln_observations, port_lincoln_tourism, by = "Quarter") %>%
  select(
    -c(
      "scientificName",
      "taxonConceptID",
