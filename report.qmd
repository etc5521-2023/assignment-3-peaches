...
Ecotourism involves traveling to places in support of the conservation efforts of endangered wildlife. To assess the level of interest in ecotourism for the Woylie, an analysis that combines tourism data and previous findings will provide us with a holistic view. 

The tourism data will be obtained from [Tourism Research Australia (TRA)](https://www.tra.gov.au) and will be used to analyse sightings close to the weather stations of interest, as reported in @sec-ref. The focus will be on one of the prominent regions where the Woylie is found: the SA2 region of Narrabri Surrounds.

## Analysis of tourists in Narrabri Surrounds 
...
pl1 <-
  ...
  geom_line() + geom_point() + theme_classic() + ggtitle("Tourism trends in Narrabri Surrounds") + labs(
    x = "Year (Quarter)",
...
ggplot(annual_data, aes(x = Year, y = TotalTrips)) +
  ...
  geom_point(size=3) +
  geom_smooth(method='loess', color="red", se=FALSE) +  # Add a smoothing curve
  annotate("text", x=2002, y=75, label="Peak in 2005", vjust=-1) +  # Adjusted positioning
  annotate("text", x=2016, y=75, label="Peak around 2018", vjust=-1) +  # Adjusted positioning
  expand_limits(y=0) +  # Adjust y-axis
  labs(title = "Enhanced Annual Trend of Trips",
       x = "Year",
       y = "Total Trips") +
  theme_minimal() +
...
#| fig-cap: "Seasonal decomposition of holiday trips in the Narrabri region"
