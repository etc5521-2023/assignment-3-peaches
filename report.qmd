...
-   This helps us assess whether the population is declining as
    expected.
-   We see that the actual trend is not aligning with our expectation, we see a       
    relatively flat trend with minimal sightings from around 2000 up until            
    approximately 2015 and significant spike in sightings around 2020.
...
hp <- ggplot(temporal_data, aes(x = Quarter, y = Year, fill = Trips)) +
  geom_tile() +
  scale_fill_gradient(low = "white", high = "darkred") +
  labs(title = "Seasonal Patterns Across Years", x = "Quarter", y = "Year") +
  theme_classic()
hp_plotly <- ggplotly(hp)
hp_plotly

```
@fig-temporal2 represents seasonal patterns from the years 1998-2022
...
#| label: fig-temporal3
#| fig-cap: "Total Trips per Quarter"
#| 
...
1. The third quarter (Q3) is the most active period, suggesting factors or events in these months (July to September) that drive an increase in the number of trips.

2. There is a clear progression of activity from Q1 to Q3, starting with the lowest activity in Q1 and gradually increasing to reach its pinnacle in Q3.
...
3.Holiday trips gradually increase from Q1 to Q2 and surge in Q3.
