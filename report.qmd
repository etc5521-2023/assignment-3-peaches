...
Based on the available data which is obtained from [Tourism Research Australia (TRA)](https://www.tra.gov.au), we will focus our analysis on two critical locations. These are:
...
```{r}
#| label: fig-mallee
#| fig-cap: "Number of holiday trips in Mallee region"
locs <- c("Port Lincoln","Mallee Region nfd")
purpose <- c("Holiday","Visiting")
tourism_woylie <- tourism %>% filter(Region %in% locs,Purpose == "Holiday")

tourism_pl <- tourism_woylie %>% filter(Region == "Mallee Region nfd")
...
pl1 <- ggplot(data = tourism_pl_grouped,aes(x = Quarter,y = Total_trips)) + 
  geom_line() + geom_point() + theme_classic() + ggtitle("Tourism trends in Mallee Region") + labs(x = "Year (Quarter)",
                                                                                                      y = "Total holiday trips",
                                                                                                      caption = "Source: Tourism Research Australia",
                                                                                                      subtitle = "1998 Q1 - 2022 Q4")
ggplotly(pl1)
```
Based on our initial analysis of @fig-mallee, we can observe the following:
